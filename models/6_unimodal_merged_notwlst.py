...
if 1 in feature_set:
    X = FMRI_FEATURES
    title = 'FMRI (merged)'
    result_df = test_input(df_all, X, y, title=title, model=model, cv=cv)
...
if 5 in feature_set:
    ...
    result_df = test_input(
        df_all, X, y, title=title, model=model, cv=cv, confounds=confounds)
...
all_dfs.to_csv(
    out_path / f'6_unimodalmergednotwlst_{model}_{y}_{suffix}.csv', sep=';')
